            ...
            {alerts?.map((alertData) => {
                 return ( ( select == 'myAlert'  && alertData.email == authData?.email) || (select == 'alert'  && alertData.email != authData?.email) ) && (
                  <View style={styles.notifyBox}>
                            <View style={ (alertData.finished_lat == 0) ? styles.notifyContentEmpty : styles.notifyContent}>
                              <View style={{
                                flexDirection: 'row',

                              }}>
                                <Notification icon="" name={ alertData.email + '\n' }  />
                              </View>
                            ...
                            <NofityCars icon="car" name={alertData.car?.brand + " - " + alertData.car?.model} subTitle={alertData.car?.licensePlate + " - " + alertData.car?.color} />
                            ...
                            >
                              {(() => {
                                if(((alertData.finished_lat ?? 0) != 0 && (alertData.finished_log ?? 0) != 0)) {
                                  return(
                                    <Text
                                    style={{
                                        fontSize: 18,
                              ...
                              {(() => {
                                if((alertData.finished_lat ?? 0) != 0 && (alertData.finished_log ?? 0) != 0) {
                                  return (
                                    ...
                                    >
                                    {(alertData.status == 1 && (alertData.finished_lat ?? 0) != 0
                                    && (alertData.finished_log ?? 0) != 0) && (<MapView
                                        provider={PROVIDER_GOOGLE} // remove if not using Google Maps
                                        style={styles.map}
                                        initialRegion={{
                                            latitude: Number(alertData.finished_lat ?? 0),
                                            longitude: Number(alertData.finished_log ?? 0),
                                            latitudeDelta: 0.003,
                                            longitudeDelta: 0.003,
                                        }}
                                    >
                                        <Marker
                                            coordinate={{ latitude: (Number(alertData.finished_lat ?? 0)),
                                              longitude: (Number(alertData.finished_log ?? 0))}}
                                        />
                                    </MapView>)}

                                    {(() => {
                                      if (alertData.status == 0 && (alertData.lat ?? 0) != 0 && (alertData.log ?? 0) != 0) {
                                        return (
                                          <MapView
                                              provider={PROVIDER_GOOGLE} // remove if not using Google Maps
                                              style={styles.map}
                                              region={{
                                                  latitude: Number(alertData?.lat ?? 0),
                                                  longitude: Number(alertData?.log ?? 0),
                                                  latitudeDelta: 0.003,
                                                  longitudeDelta: 0.003,
                                              }}
                                          >
